...
function extractOutputText(response: any): string {
  ...
  const pieces: string[] = [];
  for (const item of response?.output ?? []) for (const content of item?.content ?? []) if (content?.type === "output_text" && typeof content.text === "string") pieces.push(content.text);
  return pieces.join("");
}
...
function baseQuestionSchema(sourceIds: string[], objectiveIds: string[], conceptIds: string[]) {
  return { type: "object", additionalProperties: false, required: ["id", "semanticKey", "prompt", "answers", "topic", "conceptIds", "masteryConcept", "learningObjectiveId", "difficulty", "learningStage", "explanation", "sourceId", "sourceReference", "sourceEvidence"], properties: {
    ...
    topic: { type: "string" }, conceptIds: { type: "array", minItems: 1, items: { type: "string", enum: conceptIds } }, masteryConcept: { type: "string", enum: conceptIds }, learningObjectiveId: { type: "string", enum: objectiveIds }, difficulty: { type: "string", enum: ["beginner", "intermediate", "advanced"] }, learningStage: { type: "string", enum: ["recognition", "understanding", "application"] }, explanation: { type: "string" }, sourceId: { type: "string", enum: sourceIds }, sourceReference: { type: "string" },
...
function normalizeDrafts(result: any, request: any, verificationTier: "classroom" | "high-assurance") {
  const sourceIds = new Set((request.curriculum?.sources ?? []).map((s: any) => s.id)); const objectiveIds = new Set((request.curriculum?.learningObjectives ?? []).map((o: any) => o.id)); const conceptIds = new Set((request.curriculum?.concepts ?? []).map((c: any) => c.id));
  const accepted: any[] = []; let rejectedCount = 0; const warnings: string[] = [];
  for (const [index, raw] of (result?.drafts ?? []).entries()) {
    const answers = Array.isArray(raw.answers) ? raw.answers : []; const evidence = raw.sourceEvidence;
...
Deno.serve(async (req) => {
  ...
  const apiKey = Deno.env.get("OPENAI_API_KEY"); if (!apiKey) return json({ message: "OPENAI_API_KEY is not configured on the server." }, 500);
  let request: any; try { request = await req.json(); } catch { return json({ message: "Invalid JSON request." }, 400); }
  const sourceText = typeof request?.sourceText === "string" ? request.sourceText.trim() : ""; const curriculum = request?.curriculum; const targetQuestionCount = Math.min(Math.max(Number(request?.targetQuestionCount) || 8, 3), MAX_QUESTIONS); const verificationTier = request?.verificationTier === "high-assurance" ? "high-assurance" : "classroom";
  if (!curriculum || !sourceText) return json({ message: "Curriculum and source text are required." }, 400); if (sourceText.length > MAX_SOURCE_CHARS) return json({ message: `Source is too large for this beta (${MAX_SOURCE_CHARS} character limit).` }, 413); if (!Array.isArray(curriculum.sources) || !Array.isArray(curriculum.concepts) || !Array.isArray(curriculum.learningObjectives)) return json({ message: "Curriculum structure is invalid." }, 400);
  const sourceIds = curriculum.sources.map((s: any) => s.id); const objectiveIds = curriculum.learningObjectives.map((o: any) => o.id); const conceptIds = curriculum.concepts.map((c: any) => c.id); const questionSchema = baseQuestionSchema(sourceIds, objectiveIds, conceptIds);
  ...
  let result: any; try { result = await callStructuredModel(apiKey, systemPrompt, JSON.stringify({ curriculum: { id: curriculum.id, title: curriculum.title, sources: curriculum.sources, concepts: curriculum.concepts, learningObjectives: curriculum.learningObjectives }, targetQuestionCount, verificationTier, sourceText }), generationSchema, "grounded_question_stems"); } catch (error) { return json({ message: error instanceof Error ? error.message : "Question generation failed." }, 502); }
  ...
  let assembled: any; try { assembled = await callStructuredModel(apiKey, assemblyPrompt, JSON.stringify({ sourceText, qualified }), assemblySchema, "assembled_quality_questions"); } catch { return json({ message: "Final question assembly failed." }, 502); }
  const normalized = normalizeDrafts(assembled, request, verificationTier); normalized.rejectedCount += Math.max(0, (result.drafts ?? []).length - qualified.length);

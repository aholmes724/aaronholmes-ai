...
const HARNESS_VERSION = "1.9.0";
const QUALITY_RECIPE_VERSION = "1.8.8";
...
type VerificationTier = "classroom" | "high-assurance";

type Stage = "stems" | "pool" | "judge" | "finalize";
...
function normalizeDrafts(result: any, request: any, verificationTier: VerificationTier) {
  ...
  for (const [index, raw] of (result?.drafts ?? []).entries()) {
    ...
    const valid =
      ...
      evidence &&
      evidence.sourceId === raw.sourceId &&
      typeof evidence.excerpt === "string" &&
      evidence.excerpt.trim().length >= 12 &&
...
function buildDiagnostics(result: any, pools: any, scores: any, targetQuestionCount: number, stageTrace: string[]) {
  ...
  return {
    ...
    batching: {
      ...
      poolBatches: Math.ceil((result?.drafts?.length ?? 0) / BATCH_SIZE),
      judgeBatches: Math.ceil((result?.drafts?.length ?? 0) / BATCH_SIZE),
      executionMode: "multi-request",
    },
    stageTrace,
    threshold: {
    ...
    models: {
      ...
      finalAssembly: "deterministic code",
    },
    questions,
    summary: {
      ...
      alternativeCorrectCandidatesRejected: questions.reduce(
        (n: number, q: any) => n + q.candidates.filter((c: any) => c.alternativeCorrectness !== "clearly-wrong").length,
        0,
      ),
    },
  };
    ...
    const answers = [
      ...
      ...selected.map((candidate: any, index: number) => {
        ...
        return {
          ...
          text: candidate.text,
          correct: false,
          feedback: original?.misconception
            ? `Not quite. ${original.misconception}`
            : "This option reflects a plausible misconception, but it does not best answer the question.",
        };
      }),
    ];
...
function validateBaseRequest(request: any) {
  const sourceText = typeof request?.sourceText === "string" ? request.sourceText.trim() : "";
  const curriculum = request?.curriculum;
  if (!curriculum || !sourceText) return { error: "Curriculum and source text are required." };
  if (sourceText.length > MAX_SOURCE_CHARS) return { error: `Source is too large for this beta (${MAX_SOURCE_CHARS} character limit).`, status: 413 };
  if (!Array.isArray(curriculum.sources) || !Array.isArray(curriculum.concepts) || !Array.isArray(curriculum.learningObjectives)) {
    return { error: "Curriculum structure is invalid." };
  }
  return { sourceText, curriculum };
}
...
Deno.serve(async (req) => {
  ...
  const stage = (request?.stage ?? "stems") as Stage;
  const stageTrace: string[] = [];
  const mark = (entry: string) => {
    stageTrace.push(entry);
    console.log(`[${HARNESS_VERSION}] ${stage}:${entry}`);
  };

  const base = validateBaseRequest(request);
  if (base.error) return json({ message: base.error }, base.status ?? 400);
  const { sourceText, curriculum } = base;
  const targetQuestionCount = Math.min(Math.max(Number(request?.targetQuestionCount) || 8, 3), MAX_QUESTIONS);
  const verificationTier: VerificationTier = request?.verificationTier === "high-assurance" ? "high-assurance" : "classroom";
  ...
  if (stage === "stems") {
    ...
    try {
      ...
      mark(`complete:${result?.drafts?.length ?? 0}`);
      return json({ ok: true, stage, result, targetQuestionCount, verificationTier, stageTrace });
    } catch (error) {
      return json({ message: error instanceof Error ? error.message : "Question generation failed.", stage, stageTrace }, 502);
    }
  }

  if (stage === "pool") {
    const questions = Array.isArray(request?.questions) ? request.questions : [];
    if (!questions.length || questions.length > BATCH_SIZE) return json({ message: `Pool stage requires 1-${BATCH_SIZE} questions.` }, 400);
    ...
    mark("start");
    try {
      const pools = await callStructuredModel(
        apiKey,
        poolPrompt,
        JSON.stringify({ sourceText, questions }),
        poolSchema(questions.length),
        "distractor_candidate_pools",
        { model: DISTRACTOR_MODEL },
      );
      mark(`complete:${pools?.pools?.length ?? 0}`);
      return json({ ok: true, stage, pools, stageTrace });
    } catch (error) {
      return json({ message: "Distractor candidate generation failed.", detail: error instanceof Error ? error.message : undefined, stage, stageTrace }, 502);
    }
  }

  if (stage === "judge") {
    const questions = Array.isArray(request?.questions) ? request.questions : [];
    const pools = Array.isArray(request?.pools) ? request.pools : [];
    if (!questions.length || questions.length > BATCH_SIZE) return json({ message: `Judge stage requires 1-${BATCH_SIZE} questions.` }, 400);
    ...
    mark("start");
    try {
      const scores = await callStructuredModel(
        apiKey,
        scorePrompt,
        JSON.stringify({ sourceText, questions, pools }),
        scoreSchema(questions.length),
        "distractor_candidate_scores",
        { reasoningEffort: "high" },
      );
      mark(`complete:${scores?.questions?.length ?? 0}`);
      return json({ ok: true, stage, scores, stageTrace });
    } catch (error) {
      return json({ message: "Distractor scoring failed.", detail: error instanceof Error ? error.message : undefined, stage, stageTrace }, 502);
    }
  }

  if (stage === "finalize") {
    const result = request?.result;
    const pools = request?.allPools;
    const scores = request?.allScores;
    const fullTrace = Array.isArray(request?.stageTrace) ? request.stageTrace : [];
    mark("start");
    if (!result || !Array.isArray(result.drafts) || !pools || !scores) return json({ message: "Finalize stage is missing intermediate generation state." }, 400);
    ...
    if (result.suitability === "blocked") {
      return json({
        ...
        provider: "openai",
        model: DEFAULT_MODEL,
        stageTrace: [...fullTrace, ...stageTrace],
      });
    }

    const diagnostics = buildDiagnostics(result, pools, scores, targetQuestionCount, [...fullTrace, ...stageTrace]);
    const assembled = assembleDeterministically(result, pools, scores);
    mark(`assembly:${assembled.drafts.length}`);
    ...
    if (!assembled.drafts.length) {
      return json({
        ...
        diagnostics: {
          ...diagnostics,
          stageTrace: [...fullTrace, ...stageTrace],
          finalAssembly: { mode: "deterministic", qualifiedQuestionIds: [], acceptedQuestionIds: [], selectedDistractors: {} },
        },
      });
    }
    ...
    mark(`validation:${normalized.accepted.length}`);

    const finalTrace = [...fullTrace, ...stageTrace];
    const finalDiagnostics = {
      ...diagnostics,
      stageTrace: finalTrace,
      finalAssembly: {
        mode: "deterministic",
        qualifiedQuestionIds: assembled.qualifiedQuestionIds,
        acceptedQuestionIds: [...acceptedIds],
        selectedDistractors: assembled.selectedByQuestion,
        validationRejections: normalized.rejected,
      },
    };
    ...
    return json({
      ...
      suitability: result.suitability,
      message: `${normalized.accepted.length} question${normalized.accepted.length === 1 ? "" : "s"} assembled after resumable multi-request qualification.`,
      provider: "openai",
      model: DEFAULT_MODEL,
      distractorModel: DISTRACTOR_MODEL,
      verificationTier,
  ...
  return json({ message: `Unknown generation stage: ${String(stage)}` }, 400);
});